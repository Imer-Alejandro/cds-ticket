...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!hasPermission(session, 'settings.teams.edit')) return NextResponse.json({ error: 'Permisos insuficientes' }, { status: 403 })
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const session = await getSession()
    if (!session) return NextResponse.json({ error: 'No autorizado' }, { status: 401 })
    if (!hasPermission(session, 'settings.teams.delete')) return NextResponse.json({ error: 'Permisos insuficientes' }, { status: 403 })
