...
export default function LabelsPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { setNewName(""); setNewColor("#2563eb"); fetchLabels() }
    } catch (error) {
  ...
  const startEdit = (label: Etiqueta) => {
    setEditingId(label.id)
    setEditName(label.nombre)
  ...
  const saveEdit = async (id: string) => {
    try {
      const res = await fetch(`/api/labels/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nombre: editName, color: editColor })
      })
      if (res.ok) { cancelEdit(); fetchLabels() }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-[1fr_300px]">
        <Card>
          <CardHeader><CardTitle>Listado de Etiquetas</CardTitle></CardHeader>
          <CardContent>
            {loading ? (
                      ...
                      {editingId === label.id ? (
                        <>
                          ...
                          <TableCell>
                            <div className="flex gap-2">
                              ...
                              <Input value={editColor} onChange={(e) => setEditColor(e.target.value)} className="h-8 w-24" />
                            </div>
                          </TableCell>
                          <TableCell>{label._count?.tickets || 0}</TableCell>
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-1">
                              <Button variant="ghost" size="icon" className="text-green-600" onClick={() => saveEdit(label.id)}>
                        ...
                        <>
                          <TableCell className="font-medium">
                            <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold" style={{ backgroundColor: `${label.color}20`, color: label.color }}>
                              {label.nombre}
                            </span>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-1">
                              <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-primary" onClick={() => startEdit(label)}>
                                <Pencil className="h-4 w-4" />
                              </Button>
                              <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-destructive" onClick={() => handleDelete(label.id)}>
          ...
          <CardContent>
            <form onSubmit={handleCreate} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input placeholder="Ej. VIP" value={newName} onChange={(e) => setNewName(e.target.value)} required />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">Color</label>
                <div className="flex gap-2">
                  <Input type="color" value={newColor} onChange={(e) => setNewColor(e.target.value)} className="w-12 h-10 p-1" />
                  <Input value={newColor} onChange={(e) => setNewColor(e.target.value)} pattern="^#[0-9A-Fa-f]{6}$" required />
                </div>
              </div>
